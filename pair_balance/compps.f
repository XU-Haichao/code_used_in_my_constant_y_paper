...
c jp120504      MAXSC=50
c kaa TAU is undefined at this point so set to zero
      TAU = 0.0
...
C 
      ...
      DOUBLE PRECISION, ALLOCATABLE, SAVE ::  BGPL1(:,:,:),
     2     BGMI1(:,:,:), BGPL2(:,:,:),BGMI2(:,:,:), BGMIN(:,:,:),
     3     BGMEX(:,:,:), BGPLA(:,:,:),BGMIA(:,:,:),
     4     BGMINA(:,:,:)
      DOUBLE PRECISION, ALLOCATABLE, SAVE :: CINP(:,:,:),
     1     CINM(:,:,:),CEMP(:,:,:), CEMM(:,:,:),CGRIN(:,:,:),
     2     CGREX(:,:,:)
      DOUBLE PRECISION, ALLOCATABLE :: SOUPL(:,:),SOUMI(:,:)
      DOUBLE PRECISION, ALLOCATABLE :: SOUPL0(:,:),SOUMI0(:,:)
      DOUBLE PRECISION, ALLOCATABLE :: DITAPL0(:,:),DITAMI0(:,:) 
      DOUBLE PRECISION TAUVEC(MAXTAU)
     ...
     1 DIAVER(MAXFRE,MAXTAU), DIFLUX(MAXFRE,MAXTAU) 
      ...
      DOUBLE PRECISION, SAVE :: T0TAUGOLD,TAUOLD,YOLD,FIL1OLD,YPHOLD,PI
      ...
      if ( first ) then
         ALLOCATE(BGPL1(MAXTAU,MAXTAU,KK),BGMI1(MAXTAU,MAXTAU,KK),   
     2        BGPL2(MAXTAU,MAXTAU,KK),BGMI2(MAXTAU,MAXTAU,KK), 
     3        BGMIN(MAXTAU,MAXTAU,KK),BGMEX(MAXTAU,MAXTAU,KK), 
     4        BGPLA(MAXTAU,MAXTAU,MAXFRE),BGMIA(MAXTAU,MAXTAU,MAXFRE),
     5        BGMINA(MAXTAU,MAXTAU,MAXFRE))
         ALLOCATE(CINP(MAXTAU,MAXTAU,MAXANG),CINM(MAXTAU,MAXTAU,MAXANG),
     2        CEMP(MAXTAU,MAXTAU,MAXANG), CEMM(MAXTAU,MAXTAU,MAXANG),
     3        CGRIN(MAXTAU,MAXTAU,MAXANG), CGREX(MAXTAU,MAXTAU,MAXANG))
         first = .false.
      endif

      ALLOCATE(SOUPL(MAXTAU,LL),SOUMI(MAXTAU,LL))
      ALLOCATE(SOUPL0(MAXTAU,LL),SOUMI0(MAXTAU,LL))
      ALLOCATE(DITAPL0(LL,MAXTAU),DITAMI0(LL,MAXTAU))
...
c emergent Comptonized radiation field through sides 
 ...
 380     CONTINUE
      ...
      DEALLOCATE(SOUPL,SOUMI,SOUPL0,SOUMI0,DITAPL0,DITAMI0)
