...
struct RunStats {
    explicit RunStats(const ic::Config& cfg)
        : energy_hist(cfg.energy_bins, cfg.energy_min, cfg.energy_max),
          mu_hist(cfg.mu_bins, -1.0, 1.0),
          mu_hist_up_scattered(cfg.mu_bins, -1.0, 1.0),
          mu_hist_down_scattered(cfg.mu_bins, -1.0, 1.0),
          energy_hist_up_scattered(cfg.energy_bins, cfg.energy_min, cfg.energy_max),
          energy_hist_down_scattered(cfg.energy_bins, cfg.energy_min, cfg.energy_max),
...
std::string make_run_tag(const ic::Config& cfg) {
    ...
    const std::string transport_suffix =
        cfg.transport_cross_section == "thomson"
            ? ""
            : "_tc_" + sanitize_token(cfg.transport_cross_section);
    return prefix +
           "mode_" + sanitize_token(cfg.mode) +
           "_gm_" + sanitize_token(cfg.geometry_model) +
           "_em_" + sanitize_token(cfg.electron_model) +
           "_pe_" + sanitize_token(format_number(cfg.incident_photon_energy)) +
           "_ge_" + sanitize_token(format_number(cfg.electron_gamma)) +
           "_kte_" + sanitize_token(format_number(cfg.electron_kTe)) +
           "_sh_" + sanitize_token(format_number(cfg.slab_height)) +
           "_st_" + sanitize_token(format_number(cfg.slab_optical_depth)) +
           "_si_" + sanitize_token(cfg.slab_injection_model) +
           "_ms_" + std::to_string(cfg.max_scatters) +
           transport_suffix +
           "_seed_" + std::to_string(cfg.seed) +
           "_eb_" + std::to_string(cfg.energy_bins) +
           "_mb_" + std::to_string(cfg.mu_bins);
}
    ...
    write_key_value_csv(path,
                        {
                            ...
                            {"max_scatter_termination_fraction", format_number(::max_scatter_termination_fraction(stats))},
...
RunStats execute_run(const ic::Config& cfg) {
    ...
    for (std::uint64_t i = 0; i < cfg.num_events; ++i) {
        if (cfg.geometry_model == "slab") {
            const ic::Vec3 injection_position{0.0, 0.0, slab.z_min};
            const ic::Photon photon =
                ic::inject_photon_from_lower_boundary(cfg.incident_photon_energy,
        ...
        const ic::Photon photon = ic::make_incident_monoenergetic_photon(cfg.incident_photon_energy);
...
CompletedRun run_and_record(ic::Config cfg) {
    ...
    const std::string energy_path = prefix + "_energy_hist.csv";
    const std::string energy_up_path = prefix + "_energy_hist_up_scattered.csv";
    const std::string energy_down_path = prefix + "_energy_hist_down_scattered.csv";
    const std::string mu_path = prefix + "_mu_hist.csv";
    const std::string mu_up_path = prefix + "_mu_hist_up_scattered.csv";
    const std::string mu_down_path = prefix + "_mu_hist_down_scattered.csv";
    const std::string scatter_count_path = prefix + "_scatter_count_hist.csv";
    const std::string summary_path = prefix + "_run_summary.csv";
    const std::string config_path = prefix + "_config_snapshot.csv";
