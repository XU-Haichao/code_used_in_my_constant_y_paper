c*********************************************************************  
      subroutine xscomppscf(ear,ne,param,Ifl,photar,photer)
c*********************************************************************
c*********************************************************************
c     number of model parameters: 12
...
c
c   7: H/R for cylinder geometry only
c   8: cosIncl, cosine of inclination angle 
c      (if < 0 then only black body) 
c   9: cov_fac, covering factor of cold clouds 
c               if IGEOM=\pm 4,5 then cov_fac is dummy
c  10: redshift 
c  11: scattering fraction
c  12: maximum scattering order; 0 uses the native compPS formula
      implicit none
      integer np,i,i1,nem
      PARAMETER(nem=5000) 
      PARAMETER(np=12)
      integer ne,Ifl 
      INTEGER MAXSC_OVERRIDE, MAXSC_PARAM
      real :: param(*),ear(0:ne),photer(ne),photar(ne)
      real :: pa0(np)
      real(kind=8) :: pac(np)    
      real :: temin
      ...
      save firstcall
      data firstcall/.true./
      data    pa0/np*9999./,temin/1./
      REAL(kind=8) :: phnorm
      LOGICAL :: energiesChanged,spinChanged

      real,parameter :: YMIN=10.0!0.03333
      INTEGER :: NewBinsHigh=0
      INTEGER :: NewBinsLow=0
      REAL :: dellogEHigh = 0.01
      REAL :: dellogELow = 0.01;
      REAL :: zshift,f_sc,RestEmin,RestEmax
      INTEGER :: NewBins,nEtot,NEtot1
      REAL, ALLOCATABLE :: ExtendEnergyArray(:),ExtendFlux(:)
      REAL, ALLOCATABLE :: ExtendFluxErr(:)
      
      REAL(kind=8),ALLOCATABLE,SAVE::phcon(:),phref(:)
      REAL(kind=8),ALLOCATABLE,SAVE::phblb(:),obj(:)

      REAL(kind=8),ALLOCATABLE::spcon(:),spblb(:),spref(:)

      integer,save::ne_sav=0,nEtot_sav=0
      REAL,ALLOCATABLE,SAVE::ear_sav(:),photar_sav(:)
      REAL(kind=8)::factor
      REAL(kind=8)::spec_in(ne),e_in(0:ne),en,den,scale
      ...
      if (firstcall) then
       call xwrite('CompPS Version 4.00*',5) 
       call xwrite('Comptonization by Iterative Scattering Method',5) 
       call xwrite('Poutanen & Svensson 1996',5) 
       call xwrite('Questions: Juri Poutanen (juri.poutanen@oulu.fi)',5)
       call xwrite('*Convolution version - by Michal Szanecki',5)
       firstcall = .false.
      end if

c check if the parameters were changed  
      fl0=.false.
      do i=1,np
         if(param(i).ne.pa0(i)) fl0=.true.
         pa0(i)=param(i)
      enddo
      do i=1,np
         pac(i)=0d0
...
c gamma_max
       pac(4)=dble(param(4))
c Thomson optical depth
       pac(5)=dble(param(5))
c geometry
       pac(6)=dble(param(6))
c H/R for cylinder geometry 
       pac(7)=dble(param(7))        
...
c cov_fac 
       pac(9)=dble(param(9))
c redshift
       pac(10)=dble(param(10))
c scattering fraction
       f_sc=param(11)
c maximum scattering order override
       MAXSC_PARAM=NINT(param(12))
       IF(MAXSC_PARAM.lt.0) MAXSC_PARAM=0
       MAXSC_OVERRIDE=MAXSC_PARAM
...
!       write(*,*)f_sc
      ...
      energiesChanged=.FALSE.
      if (ne_sav.ne.ne)then
         energiesChanged=.TRUE.
         if(allocated(ear_sav))deallocate(ear_sav)
         allocate(ear_sav(0:ne))
         if(allocated(photar_sav))deallocate(photar_sav)
         allocate(photar_sav(ne))
         ne_sav=ne
      else
         i=0
         do while((ear(i).eq.ear_sav(i)).and.(i.lt.ne))
            i=i+1
         enddo
         if (i.ne.ne)energiesChanged=.TRUE.
      endif
      ear_sav = ear

      spinChanged=.FALSE.
      if(energiesChanged)then
         spinChanged=.TRUE.
      else
         i=1
         do while((photar(i).eq.photar_sav(i)).and.(i.lt.ne))
            i=i+1
         enddo
         if (i.ne.ne)spinChanged=.TRUE.            
      endif
      photar_sav=photar
      
      zshift = real(1.0+pac(10))
!     RestEmin = 0.005/zshift
      RestEmin = 1e-5/zshift      
      RestEmax = 511.0/YMIN/zshift

      if(ear(0).gt.RestEmin)NewBinsLow=
     &     INT((log10(ear(0))-log10(RestEmin))*100.0)
      if(ear(ne).lt.RestEmax)NewBinsHigh=
     &     INT((log10(RestEmax)-log10(ear(ne)))*100.0)

      NewBins = NewBinsLow + NewBinsHigh
      nEtot = nE + NewBins
      nEtot1 = nEtot + 1

      if(allocated(ExtendEnergyArray))deallocate(ExtendEnergyArray)
      allocate(ExtendEnergyArray(nEtot1))
      if(allocated(ExtendFlux))deallocate(ExtendFlux)
      allocate(ExtendFlux(nEtot))
      if(allocated(ExtendFluxErr))deallocate(ExtendFluxErr)
      allocate(ExtendFluxErr(nEtot))
      
      do i=1,NewBinsLow
         ExtendEnergyArray(i)=10.0**(log10(RestEmin)+i*dellogELow)
      enddo
      do i=NewBinsLow+1,ne+NewBinsLow+1
         ExtendEnergyArray(i)=ear(i-NewBinsLow-1)
      enddo
      do i=ne+NewBinsLow+2,nEtot1
         ExtendEnergyArray(i)= 
     &        10.0**(log10(ExtendEnergyArray(nE+NewBinsLow+1))
     &        +(i-ne-NewBinsLow-1)*dellogEHigh)
      enddo

      if(energiesChanged.or.(nEtot_sav.ne.nEtot))then
         if(allocated(phcon))deallocate(phcon)
         if(allocated(phblb))deallocate(phblb)
         if(allocated(phref))deallocate(phref)
         if(allocated(obj))deallocate(obj)
         allocate(phcon(nEtot))
         allocate(phblb(nEtot))
         allocate(phref(nEtot))
         allocate(obj(nEtot))
         nEtot_sav=nEtot
      endif
      ...
      do i=1,nEtot
         obj(i) = (1.+pac(10))*
     &        (ExtendEnergyArray(i)+ExtendEnergyArray(i+1))/2.0/511.0;
      enddo

      do i=1,ne
         spec_in(i)=dble(photar(i))
      enddo
      do i=0,ne
         e_in(i)=dble(ear(i))
      enddo
      ...
      scale=0.0
      do i=1,ne
         en=dsqrt(e_in(i-1)*e_in(i))
         scale=scale+en*spec_in(i)
      enddo
      scale=scale/511.
!      write(*,*)scale
...
!     write(*,*)spec_in(i)
      ...
      if(fl0.or.energiesChanged.or.spinChanged)then
         call msismco(pac,obj,phcon,phblb,phref,nEtot,phnorm,
     &        energiesChanged,spinChanged,spec_in,e_in,ne)
      endif

       if(allocated(spcon))deallocate(spcon)
       if(allocated(spblb))deallocate(spblb)
       if(allocated(spref))deallocate(spref)
       allocate(spcon(nEtot))
       allocate(spblb(nEtot))
       allocate(spref(nEtot))              

       do i=1,nEtot
          factor=(ExtendEnergyArray(i+1)-ExtendEnergyArray(i))*phnorm
...
!       photar=0
!       phnorm=8.56495872E+09    !1@1 for default and input bb(kTbb=1.e-3, norm=1)
!       phnorm=1.0/511.0
       ...
       IF(f_sc.ge.0.)THEN
          do i=1,ne
             i1=i+NewBinsLow
             if(param(8).lt.0.) photar(i)=real(spblb(i1)*f_sc*phnorm)
             if(param(8).ge.0.) photar(i)=(1.0-f_sc)*photar(i)+
     &            f_sc*real((spblb(i1)+spcon(i1))*phnorm)
!     if(param(8).ge.0.) photar(i)=real((spcon(i1))/phnorm)
          enddo
       ELSE
          do i=1,ne
             i1=i+NewBinsLow
             if(param(8).lt.0.) photar(i)=0.0
             if(param(8).ge.0.) photar(i)=
     &            abs(f_sc)*real((spcon(i1))*phnorm)
          enddo
      ENDIF
...
!       call dbinsrch(1.0d0,i,1.0d0*ear,ne+1)
!       write(*,*)i,ear(i-1),ear(i),photar(i)/(ear(i)-ear(i-1))
      return
      end

      
c********************************************************************* 
      SUBROUTINE MSISMCO(PARM,OBJ,PHCON,PHBLB,PHREF,NE,PHNORM,FLAGFRE, 
     &      FLAGSPNC,spec_in,e_in,ne_in)
c********************************************************************* 
...
c********************************************************************* 
      ...
      DOUBLE PRECISION t0taug, tau, te,yyy

      INTEGER neold, igeold, inpold
      INTEGER MAXSC_OVERRIDE, MAXSC_OVERRIDE_OLD
      INTEGER i, iang, j, jx, k, l, n, nl

      logical firstcall,flagfre,flagspnc,flagcs 
      COMMON /MSMAXSCOVR/ MAXSC_OVERRIDE
      SAVE /MSMAXSCOVR/

      save    pa0, firstcall, neold, igeold
      save    MAXSC_OVERRIDE_OLD
      ...
      data    MAXSC_OVERRIDE/0/
      data    MAXSC_OVERRIDE_OLD/-999999/
...
C ************************************
      ...
      if(firstcall.and.IEXACTANG.ne.1) IEXACTANG=0
...
cnumber of scatterings 
cjp120504      MAXSC=50 
      IF(MAXSC_OVERRIDE.gt.0) THEN
         MAXSC=MAXSC_OVERRIDE
      ELSE
...
c hybrid
      ...
      if ( IDISTF .EQ. 0 .OR. IDISTF .EQ. 3 ) THEN
         if ( ABS(PARM(1)) .LE. 1.0 ) THEN
            ...
            WRITE(comment, '(2(a,1pg14.7))')"kTe too small =", PARM(1)
...
c This line added to suppress a compiler warning
      ...
      IF(PARM(5).gt.0D0.or.IDISTF.eq.2) THEN
         TAU=DABS(PARM(5)) 
      ELSE
c tau=y/(4*Theta) 
         TAU=DABS(PARM(5)/(4D0*TE)) 
      ENDIF 
C *****************************************       
!     YPH=511D0/TPH
      YPH=511D0
...
c Compton spectral parameters have changed  
      flagcs = .false.
      do i = 1, 10
         if ( (pa0(i) .NE. parm(i)).or.flagspnc) flagcs = .true.
      enddo
      if (MAXSC_OVERRIDE.ne.MAXSC_OVERRIDE_OLD) flagcs = .true.
...
c for the central injection - Gauss-Chebyshev angular quadrature 
         if(INPUT.eq.0.and.(IGEOM.eq.4.or.IGEOM.eq.5)) then 
            CALL MSQCHEB(UANG,AANG,MAXANG)
         else 
c for any  other  injection - Gauss angular quadrature 
            CALL MSQDRGSDO(UANG,AANG,MAXANG)  
            DO 20 L=1,MAXANG
...
C NL    -  number of points in Gauss-Laguerre quadrature  
         NL=10
         CALL MSAGUEGALA(NL,EPS) 
         CALL MSQDRGSDO(UG,AG,NG)  
         DO 23 L=1,NG
            UG(L)=0.5D0*(1D0+UG(L))                         
            AG(L)=AG(L)*5D-1                                
 23      CONTINUE 
         CALL MSQDRGSDO(UGS,AGS,NS)
         DO 26 L=1,NS
            UGS(L)=0.5D0*(1D0+UGS(L))
            AGS(L)=AGS(L)*5D-1
 26      CONTINUE
         CALL MSQDRGSDO(UGSPH,AGSPH,NSPH)
         DO 28 L=1,NSPH
...
C intrinsic radiation - arbitrary component being convolved

         de=dlog10(XKEV(2))-dlog10(XKEV(1))
         DO I=0,MAXFRE-1
            ear_xkev(I)=10.0**(dlog10(XKEV(I+1))-de/2.0)
         ENDDO
         ear_xkev(MAXFRE)=10.0**(dlog10(XKEV(MAXFRE))+de/2.0)
         DINTEN=0.0
         call map_to_grid(ear_xkev,MAXFRE,e_in,ne_in,DINTEN,spec_in)

         de=0.
         DO I=1,MAXFRE
            DINTEN(I)=DINTEN(I)/(ear_xkev(I)-ear_xkev(I-1))*XKEV(I)
!            if(DINTEN(I).lt.1d-30)DINTEN(I)=1d-10
            de=de+DINTEN(I)*(ear_xkev(I)-ear_xkev(I-1))/511.
!            write(*,*)I, XKEV(I),DINTEN(I)
         ENDDO
 !        write(*,*)"norm_int=",de
         
         i=1
         DO WHILE((DINTEN(i).lt.1e-20).and.(i.lt.MAXFRE))
         i=i+1
         ENDDO
         DO j=i-1,1,-1
!     DINTEN(j)=DINTEN(i)*dexp(1.0-j/(i-1))
            DINTEN(j)=DINTEN(i)*dexp((dble(j-1)/dble(i-1)-1.0)*500.)
!            write(*,*)j, XKEV(j),DINTEN(j)
         ENDDO
         i=MAXFRE
         DO WHILE((DINTEN(i).lt.1e-20).and.(i.gt.1))
            i=i-1
         ENDDO
         DO j=i+1,MAXFRE
            DINTEN(j)=DINTEN(i)*dexp((dble(i+1)/dble(j+1)-1.0)*500.)
!            write(*,*)j,dexp((dble(i+1)/dble(j)-2.0)*100.)
         ENDDO
...
C calculation of the comptonized spectrum
      if(flagcs) then
         CALL MSCOMPSPEC(TAU,T0TAUG,FILF1)
...
c interpolation between angles to get N_nu=I_nu/nu at ETA
            ...
            DO I=1,MAXFRE
               ...
               ENDDO
               CALL MSPOLINTQ(UANG,FINT,MAXANG,ETA,FLY,DELY)
               COMPTSP(I)=DEXP(FLY)
            ENDDO
...
C******************************************************
      ...
      if(flagcs.or.flagfre.or.flagspnc) then
c calculation of COMPTON SCATTERED FLUX at obs. grid 
         DO 170 I=1,MAXFRE
            DSPXX(I)=DLOG(COMPTSP(I)+CMIN)  
 170     CONTINUE 
         CALL MSQSPLINE(XLOG,DSPXX,MAXFRE,1d30,1d30,F2) 
         DO 172 I=1,NE 
            IF(OBJ(I).lt.XEN(1).or.OBJ(I).ge.XEN(MAXFRE)) THEN 
               PHCON(I)=CMIN 
            ELSE 
               CALL MSQSPLINT(XLOG,DSPXX,F2,MAXFRE,DLOG10(OBJ(I)),YYY)
...
c calculation of BLACK BODY FLUX at obs. grid 
 ...
 176     CONTINUE          
      ...
      do n=1,10
         pa0(n)=parm(n)
      enddo
      MAXSC_OVERRIDE_OLD=MAXSC_OVERRIDE
      RETURN 
      END                  
...
C 
      SUBROUTINE MSCOMPSPEC(TAU,T0TAUG,FILF1)
      IMPLICIT NONE
...
c for disk with holes      
     ...
     &        .or.T0TAUG.ne.T0TAUGOLD)) then 
            CALL  MSENERGR(TAUVEC,TAU,CGRIN,CGREX,
     &           BGMIN,BGMEX,BGMINA)
         endif
c end IGEOM.ne.4,5      
      endif
...
c S_0(tau) homogeneous
            ...
            IF(IGEOM.eq.2.or.IGEOM.eq.3) THEN
               CALL MSSIDEESC(SOUPL0,SOUMI0,DELTAU,DNTR,FILF1,BGMEX,
     &              BGPL2,BGMI2,DINUP,DINDO)
               DO 31 L=1,LL
...
c EXACT INTENSITIES
            IF(NISO.eq.0.or.ISC.le.ISOTR) THEN 
               if(IGEOM.eq.4.or.IGEOM.eq.5)  then 
c sphere 
                  CALL MSEXASPH(SOUPL0,SOUMI0,DITAPL0,DITAMI0,DIAVER,
     &                 DIFLUX,TAUVEC,TAU,ISC)
               else
                  CALL MSEXAINT(SOUPL0,SOUMI0,DITAPL0,DITAMI0,DIAVER,
...
c ISOTROPIC INTENSITIES    
               if(IGEOM.eq.4.or.IGEOM.eq.5)  then
c sphere 
                  CALL MSISOSPH(SOISPL,SOISMI,DIAVER,DIFLUX,TAUVEC,
     &                        TAU,ISC)
               else 
                  CALL MSISOINT(SOISPL,SOISMI,DIAVER,DIFLUX, 
...
c EXACT SOURCE FUNCTIONS
            IF(NISO.eq.0.or.ISC.lt.ISOTR) THEN 
               CALL MSEXASOUR(SOUPL,SOUMI,DITAPL0,DITAMI0,SOUPL0,SOUMI0,
     &                      DIFMAX)     
            ELSE
c ISOTROPIC SOURCE FUNCTIONS dependent on TAU
               CALL MSISOSOUR(SOUPL,SOUMI,DIAVER,DIFLUX,SOISPL,SOISMI,
     &                      DIFMAX)
            ENDIF
...
c escape probability (for sphere) 
            CALL MSESCINT(SOISPL,DIAVER,TAU,ISC)
            CALL MSESCSOUR(SOUPL,DIAVER,SOISPL,DIFMAX) 
         endif 
...
c emergent Comptonized radiation field through sides 
      IF(IGEOM.eq.2.or.IGEOM.eq.3) THEN 
         CALL MSSIDEESC(SOUPL,SOUMI,DELTAU,DNTR,FILF1,BGMEX,BGPL2,
     &                BGMI2,DINUP,DINDO)
         DO 380 L=1,LL 
 ...
 380     CONTINUE
      ...
      DO ITAU=1,MAXTAU
         DO L=1,LL
            TINTPL(L,ITAU)=TINTPL(L,ITAU)*DNORM
            TINTMI(L,ITAU)=TINTMI(L,ITAU)*DNORM
         ENDDO
      ENDDO
      DNORMC=DNORM
...
c ESCAPE through the sides of hemisphere or cylinder 
      SUBROUTINE MSSIDEESC(SOUPL,SOUMI,DELTAU,DNTR,FILF1,BGMEX,BGPL2,
     &                   BGMI2,DINUP,DINDO) 
      IMPLICIT NONE
...
c emergent up-down and internal angle averaged radiation  
      SUBROUTINE MSESCINT(SOISTR,DISOTR,TAU,ISC)
      ...
      DOUBLE PRECISION MSescsphere
      EXTERNAL MSescsphere
      ...
      if(ISC.eq.1) then
         do ix=1,maxfre
            TAUX=S0(IX)*TAU
c MSescsphere computes 1-escape probability from a sphere
            ESCAPE(IX)=MSescsphere(TAUX)
         enddo
      endif                 
...
c for escape probability
      SUBROUTINE MSESCSOUR(SOUPL,DISOTR,SOISTR,DIFMAX)
 ...
 445  CONTINUE
      ...
      DO 450 IX=1,MAXFRE
         ...
         IF(XXPM.gt.1D-99.and.X.lt.XEMAX) THEN
            DIF=XXPM/SOUPL(1,L)
...
c
      DOUBLE PRECISION FUNCTION MSescsphere(X)
      ...
      if(x.gt.eps) then
         ...
         xr=1d0/x
         if(x.gt.40d0) then
            MSescsphere=1d0-0.75d0*xr*(1d0-xr2)
         else
            MSescsphere=1d0-0.75d0*xr*(1d0-xr2+(xr+xr2)*dexp(-x2))
         endif
      else
         sum=1d0/30d0
         dn=6d0
         ...
         do while(dabs(del).gt.accur)
            ...
            dn=dn+1d0
         enddo
         MSescsphere=0.75d0*x-12d0*x**2*sum
      endif
...
c exact intensities for a spherical geometry
      SUBROUTINE MSEXASPH(SOUPL0,SOUMI0,DITAPL0,DITAMI0,
...
c positive angles    
               ...
               DO 250 IS=1,NSPH
                  ...
                  DZETA=SPR/TAUPR
                  DZETABS=DABS(DZETA) 
                  CALL MSQHUNT(TAUVEC,MAXTAU,TAUPR,JTA)
                  if(JTA.eq.1.and.ISC.eq.1.and.INPUT.eq.0) JTA=JTA+1 
                  ARX1=TAUVEC(JTA)
                  ARX2=TAUVEC(JTA+1)

                  CALL MSDLOCATE(UANG,MAXANG,DZETABS,JA)
...
c negative angles
               ...
               if(ITAU.ne.MAXTAU) then
                  DO 255 IS=1,NSPH
                     ...
                     CALL MSDLOCATE(UANG,MAXANG,DZETA,JA)
                     if(JA.eq.MAXANG) JA=MAXANG-1
                     if(JA.ne.0) then
                        ARY1=UANG(JA)
                        ARY2=UANG(JA+1)
...
c for angle averaged radiation field 
c emergent up-down and internal angle averaged radiation  
      SUBROUTINE MSISOSPH(SOISPL,SOISMI,DIAVER,DIFLUX,TAUVEC,TAU,ISC)
...
c interpolate in SOISPL or SOISMI to get SOURINT at a given TAUPR
               if(SPR.ge.0d0) then 
                  CALL MSDLINEAR(TAUVEC(JTA),DLOG(SOISPL(JTA,IX)+CMIN),
     *                 TAUVEC(JTA+1),DLOG(SOISPL(JTA+1,IX)+CMIN),TAUPR,
     +                 SOURINT)
               else
                  CALL MSDLINEAR(TAUVEC(JTA),DLOG(SOISMI(JTA,IX)+CMIN),
...
c for angle averaged radiation field 
c emergent up-down and internal angle averaged radiation
      SUBROUTINE MSISOINT(SOISPL,SOISMI,DIAVER,DIFLUX, 
...
c internal angle averaged radiation field 
 ...
 311  CONTINUE
      ...
      END
c***********************************************************************          
      SUBROUTINE MSEXAINT(SOUPL0,SOUMI0,DITAPL0,DITAMI0,DIAVER,DIFLUX,
 ...
 240              CONTINUE 
               ...
               TINTPL(L,ITAU)=TINTPL(L,ITAU)+SUPL1
               TINTMI(L,LTAU)=TINTMI(L,LTAU)+SUMI1
 260        CONTINUE 
c emergent radiation field up-down 
...
c***********************************************************************
c calculations of isotropic source function 
      SUBROUTINE MSISOSOUR(SOUPL,SOUMI,DIAVER,DIFLUX,
     &      SOISPL,SOISMI,DIFMAX)
      ...
      DO IX=1,MAXFRE
         ...
         DO IANG=1,MAXANG
            ...
            DO ITAU=1,MAXTAU
               ...
               IF(ITAU.eq.MAXTAU.and.XXPM.gt.1D-99.and.X.lt.XEMAX) THEN
...
c calculations of exact source function 
      SUBROUTINE MSEXASOUR(SOUPL,SOUMI,DITAPL0,DITAMI0,SOUPL0,SOUMI0,
     &                   DIFMAX) 
 ...
 185        CONTINUE 
            IF(XXPL.gt.1D-99.and.X.lt.XEMAX) THEN
...
c geometrical factors depending of IGEOM 
      SUBROUTINE MSGEOMFAC(TAUVEC,T0TAUG,CINP,CINM,CEMP,CEMM)
...
c geometrical factors depending of IGEOM for DISK with HOLE geometry
      SUBROUTINE MSGEOMGR(TAUVEC,T0TAUG,CGRIN,CGREX) 
...
c end of HEMISPHERE      
 ...
 109  CONTINUE
      ...
      END
c***********************************************************************      
      SUBROUTINE MSENERFAC(TAUVEC,TAU,T0TAUG,CINP,CINM,CEMP,CEMM,
     &      BGPL1,BGMI1,BGPL2,BGMI2,BGPLA,BGMIA)
...
c FOR THE DISK WITH HOLES
      SUBROUTINE MSENERGR(TAUVEC,TAU,CGRIN,CGREX,
     &      BGMIN,BGMEX,BGMINA)
...
c here computation of R(x,x1,mu) 
            DO 350 IAU=1,NMU 
               ...
               FRDMU(JX,IX,IAU)=SMU 
 350        CONTINUE 
            CALL MSQSPLINE(DMUA,REDXX,NMU,1d30,1d30,F2)
...
c BEGIN CALN. OF  S+- (+mu,-mu') = S-+ (-mu,+mu') BLOCK MATRIX:
                     DMUPM=-COKL+PHKL
                     CALL MSQSPLINT(DMUA,REDXX,F2,NMU,DMUPM,SM)
c END CALN. OF  S+- (+mu,-mu') = S-+ (-mu,+mu') BLOCK MATRIX.
c BEGIN CALN. OF  S++ (+mu,+mu') = S-- (-mu,-mu') BLOCK MATRIX:
                     DMUPP=COKL+PHKL
                     CALL MSQSPLINT(DMUA,REDXX,F2,NMU,DMUPP,SP)
c END CALN. OF  S++ (+mu,+mu') = S-- (-mu,-mu') BLOCK MATRIX.
...
c***********************************************************************
C  calculation of  frequencies 
      SUBROUTINE MSFREQPOIN(XLMIN,HX)
...
c         CALL QSN(X*GM1,0,SSM,SSM1,SSM2,SM1,SM2,SM3,SM4,SM5,SM6,SM7,E) 
         CALL MSQSN(X*GP1,0,SSP,E) 
         CALL MSQSN(X*GM1,0,SSM,E) 
...
c         CALL QSN(X*GM2,0,SSM,SSM1,SSM2,SM1,SM2,SM3,SM4,SM5,SM6,SM7,E)
         CALL MSQSN(X*GP2,0,SSP,E) 
         CALL MSQSN(X*GM2,0,SSM,E) 
...
C 
      SUBROUTINE MSREDFUNC(X,X1,QM,S) 
      IMPLICIT NONE
...
c power-law electron distribution 
      IF(IDISTF.eq.2) CALL MSPOWMATR(X,X1,QM,S) 
      RETURN                                               
      END                                                  
...
C 
      SUBROUTINE MSMAXGALA(X,X1,QM,S) 
      IMPLICIT NONE
...
C need in advance calling the COMXX(X,X1) prosedure
      SUBROUTINE MSPOWMATR(X,X1,QM,S)
      IMPLICIT NONE
...
c int d(ln g)/z g^-p R(g)
         ...
         DO 10 I=1,NG
            ...
            GP=DEXP(-PLIND*DLG)
            GPZ=GP/Z*AGI
            IF(IREDF.eq.0) CALL MSRDSTRB(X,X1,QM,R)
            SUM1=SUM1+R*GPZ
 10      CONTINUE
         SUM1=SUM1*DL21
      endif
...
c eq. (3.3.3) in Nagirner & Poutanen 1994 
      DOUBLE PRECISION FUNCTION MSPS0JP(X,E) 
      IMPLICIT NONE
      ...
      ARG1=X*GZP 
      ARG2=X/GZP 
      MSPS0JP=(MSPSIJP(ARG1,E)*GZP2-MSPSIJP(ARG2,E)/GZP2)/4D0  
      RETURN
      END
c**************************************************
...
c eq. (3.3.10) in Nagirner & Poutanen 1994 
      DOUBLE PRECISION FUNCTION MSPSIJP(X,E) 
      IMPLICIT NONE
  ...
  240 MSPSIJP=P10 
      ...
      DOUBLE PRECISION FUNCTION MSGINT(X,E)    
      IMPLICIT NONE
      ...
      Z=DLOG(Y)               
      MSGINT=MSGINT+DL*Z                
      IF (X.GT.0.5D0) MSGINT=MSGINT+0.5D0*Z*Z
   ...
   30 A=0.5D0*A 
      ...
      IF (B.GT.E) GO TO 30   
      U=U*Y*Y                
      IF (X.GT.0.5D0) MSGINT=MSGINT-U  
      IF (X.LT.0.5D0) MSGINT=MSGINT+U  
      RETURN                 
   ...
   50 DN=DN+1D0  
      B=A*B      
      MSGINT=MSGINT+B/(DN*DN)  
      IF (DABS(B).GT.E) GO TO 50 
      MSGINT=Y*MSGINT          
      IF (X.LT.XT) RETURN  
      Z=DLOG(Y)     
      MSGINT=PIH+0.5D0*Z*Z-MSGINT     
      RETURN       
      END         
...
C A,B: coefficients of the linear function Ax+B
      SUBROUTINE MSDLININT(FUNLOG,XLOG,A,B,NMIN,NMAX,MAXFRE)
      IMPLICIT NONE
 ...
 15      CONTINUE
      ...
      END
c***********************************************************************
      SUBROUTINE MSQSPLINE(X,Y,N,YP1,YPN,Y2)
      IMPLICIT NONE
 ...
 12   CONTINUE
      ...
      SUBROUTINE MSQSPLINT(XA,YA,Y2A,N,X,Y)
      IMPLICIT NONE
...
c polinomial interpolation and extrapolation    
      SUBROUTINE MSPOLINTQ(XA,YA,N,X,Y,DY)
      IMPLICIT NONE
...
c**************************************************
c linear interpolation 
      SUBROUTINE MSDLINEAR(X1,Y1,X2,Y2,X,Y) 
      IMPLICIT NONE
...
c**********************************************************************
      SUBROUTINE MSQHUNT(XIN,N,X,JLO)
      IMPLICIT NONE
 ...
 3    IF(JHI-JLO.EQ.1)RETURN
      ...
      END
c**************************************************
      SUBROUTINE MSDLOCATE(XIN,N,X,J)
      IMPLICIT NONE
...
c**************************************************
c sin(x)/x  
      DOUBLE PRECISION FUNCTION MSSINXX(X) 
      IMPLICIT NONE
...
C 
      SUBROUTINE MSQFQLAG0 (K,EPS,T,A)    
      IMPLICIT NONE
...
C
      ...
      DOUBLE PRECISION MSDBK0LE, MSDBI0LE, MSDBK0GS
      DOUBLE PRECISION MSDBK1LO, MSDBI1LO, MSDBK1GS      
      EXTERNAL MSDBK0LE,MSDBI0LE,MSDBK0GS,MSDBK1LO,MSDBI1LO,MSDBK1GS
...
C       CALCULATION OF K(0,X)
      IF (X.LE.X1) BK(1)=MSDBK0LE(X) 
      IF (X.LE.X1) BK(1)=BK(1)-(EILER+DLOG(X/2D0))*MSDBI0LE(X)
      IF (X.GT.X1) BK(1)=MSDBK0GS(X)                          
      IF (N.EQ.1) RETURN                                    
C CALCULATION OF K(1,X)
      IF (X.LE.X1) THEN
         BK(2)=MSDBK1LO(X) 
         ED=EILER+DLOG(X/2D0) 
         BK(2)=ED*MSDBI1LO(X)+C1/X-BK(2) 
      ENDIF
      IF (X.GT.X1) BK(2)=MSDBK1GS(X)               
      IF (N.EQ.2) RETURN                         
...
C  P.511
      DOUBLE PRECISION FUNCTION MSQDCHEB0(A,X,N,KEY)   
      IMPLICIT NONE
...
C  REACTION ON THE NONPOSITIVE VALUE OF KEY
      ...
      DO 10 I=1,N1                         
         ...
         B0=Z*B1-B2+A(K+1)                  
 10   CONTINUE                             
      IF (KEY.EQ.0) MSQDCHEB0=B0-X*B1         
      IF (KEY.EQ.1) MSQDCHEB0=X*(B0-B1)       
      IF (KEY.EQ.2) MSQDCHEB0=B0-B1*Z/C2      
      IF (KEY.EQ.3) MSQDCHEB0=B0-B1*Z/C2      
      RETURN                               
      END                                       
...
C TABLE  9.5  ; P.350
C COEFFICIENTS C
      DOUBLE PRECISION FUNCTION MSDBK0LE(X)    
      IMPLICIT NONE

      DOUBLE PRECISION X, W

      DOUBLE PRECISION MSQDCHEB0
      EXTERNAL MSQDCHEB0
...
C TABLE  9.6  ; P.352
C COEFFICIENTS C 
      DOUBLE PRECISION FUNCTION MSDBK1LO(X)                      
      IMPLICIT NONE
...
C TABLE  9.5  ; P.351
C COEFFICIENTS D
      DOUBLE PRECISION FUNCTION MSDBK0GS(X)      
      IMPLICIT NONE
...
C TABLE  9.6  ; P.351
C     COEFFICIENTS   E  
      DOUBLE PRECISION FUNCTION MSDBK1GS(X)                      
      IMPLICIT NONE
...
C                "MIR",MOSCOW  1980 608 P.
C TABLE  9.5  ; P.350
      DOUBLE PRECISION FUNCTION MSDBI0LE(X)                      
      IMPLICIT NONE
...
C                "MIR",MOSCOW  1980 608 P.
C TABLE  9.6  ; P.352
      DOUBLE PRECISION FUNCTION MSDBI1LO(X)                      
      IMPLICIT NONE
...
C                                             
      ...
      do i=1,nne
        ...
        if((new_en(i).le.old_en(0)).or.(new_en(i-1).ge.old_en(one)))then
         new_flu(i) = 0.d0
         else
     ...
     &              *old_flu(bmin+1)
         ...
         dmin=(old_en(bmin+1)-new_en(i-1))/(old_en(bmin+1)-old_en(bmin))
